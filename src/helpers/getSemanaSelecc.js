...
export const getSemanaSelecc = (diaSelecc,yearSelecc, mesesDetalles,mesSelecc) => {
    ...
    return semanaActual.map((nume,index)=>{
        ...
        if(dia < 1){
            if (mesSelecc === 0) yearPertenece = yearPertenece - 1;
            return ({
                numero: numDiasMesAnterior + (incioMesAnterior+index+1),
                activo: false,
                yearPertenece,
                mesPertenece: mesPertenece - 1,
            })
            
        }else {
            if(dia > numDias){
                if (mesSelecc === 11) yearPertenece = yearPertenece + 1
                return ({
                    numero: dia-numDias,
                    activo: false,
                    yearPertenece,
